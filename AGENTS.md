...
- `plugin_env`：注入到**所有**插件进程的环境变量
- `plugin_controls.<plugin_id>.env`：仅注入到指定插件进程，**覆盖**同名全局项
- 合并顺序：宿主进程环境 < `plugin_env` < 插件专属 `env`
- 与配置模板 `${env:NAME}` 不同：模板读取的是**宿主**环境；此处写入的是**插件子进程**环境
- 通过 WebUI 或 API 修改后，受影响的插件进程会立即重启以生效
...
- `GET /api/plugin-env` — 获取全局插件环境变量
- `PUT /api/plugin-env` — 更新全局插件环境变量（变更后重启所有插件进程）
- `GET /api/plugins` — 获取插件列表
- `PUT /api/plugins/{id}/switches` — 更新插件开关/访问控制/专属 env（env 变更后重启该插件）
