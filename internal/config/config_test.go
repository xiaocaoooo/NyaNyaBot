...
func TestAccessControl(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "user in whitelist and blacklist",
			ac:      AccessControl{WhiteListUsers: []int64{123}, BlackListUsers: []int64{123}},
			userID:  123,
			groupID: 456,
			want:    true, // Whitelist has higher priority than blacklist!
		},
		{
			name:    "user not in whitelist but in blacklist",
			ac:      AccessControl{WhiteListUsers: []int64{999}, BlackListUsers: []int64{123}},
			userID:  123,
			groupID: 456,
			want:    false,
		},
		{
			name:    "user not in whitelist, not in blacklist, group in whitelist",
			ac:      AccessControl{WhiteListUsers: []int64{999}, BlackListUsers: []int64{888}, WhiteListGroups: []int64{456}},
			userID:  123,
			groupID: 456,
			want:    true,
		},
		{
			name:    "user not in whitelist, in blacklist, group in whitelist",
			ac:      AccessControl{WhiteListUsers: []int64{999}, BlackListUsers: []int64{123}, WhiteListGroups: []int64{456}},
			userID:  123,
			groupID: 456,
			want:    false, // User Blacklist has higher priority than Group Whitelist!
		},
		{
			name:    "user not in whitelist, not in blacklist, group in blacklist",
		...
		{
			name:    "group in whitelist and group in blacklist",
			ac:      AccessControl{WhiteListGroups: []int64{456}, BlackListGroups: []int64{456}},
			userID:  123,
			groupID: 456,
			want:    true, // Group Whitelist > Group Blacklist
		},
		{
			name:    "no whitelist, group in blacklist",
			ac:      AccessControl{BlackListGroups: []int64{456}},
			userID:  123,
			groupID: 456,
			want:    false,
...
func TestAppConfigIsAllowed(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "global blacklist mismatch",
			pluginID:   "any",
			listenerID: "any",
			isCommand:  true,
			userID:     123,
